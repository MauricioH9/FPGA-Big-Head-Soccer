...
// Split Luffy's theme into an array
Note luffy_theme[] = {
  {NOTE_FS4, EIGHTH},
  {NOTE_G4,  EIGHTH},
  {NOTE_A4,  QUARTER},
...
const int luffy_length = sizeof(luffy_theme) / sizeof(Note);
...
void starting_splash_screen(OsdCore *osd_p, Ps2Core *ps2_p) {
    ...
    bool start_theme = false;
    int note_index = 0;
    const int mario_len = 11;
    Note mario_intro[] = {
        {NOTE_E5, SIXTEENTH}, {NOTE_E5, SIXTEENTH}, {REST, SIXTEENTH},
        {NOTE_E5, SIXTEENTH}, {REST, SIXTEENTH}, {NOTE_C5, SIXTEENTH},
        {NOTE_E5, SIXTEENTH}, {REST, SIXTEENTH}, {NOTE_G5, EIGHTH},
        {REST, EIGHTH}, {NOTE_G4, QUARTER}
    };
    ...
    while (true) {
        ...
        // Check for button press to start theme
        if (!start_theme && !ps2_p->rx_fifo_empty()) {
            (void)ps2_p->rx_byte();  // Clear key
            start_theme = true;
        }

        // If theme started, play notes with flashing
        if (start_theme && note_index < mario_len) {
            play_note(mario_intro[note_index].freq, mario_intro[note_index].duration);
            note_index++;
        } else if (start_theme && note_index >= mario_len) {
            break;  // Done with theme, exit splash
        }
...
void game_loop() {
  ...
  while (!game_over) {
    ...
    if ((now_ms() - start_time) >= 20000) { // change time here
      game_over = true;
      //play_luffy_theme(); // plays when game is done
    }
    sleep_ms(30);
  }
  // Game over message
  ...
  // Interleave Luffy theme and flashing
  for (int i = 0; i < luffy_length; i++) {
    // Flash winner message while music plays
    for (int j = 0; j < win_len; j++)
      osd.wr_char(win_x + j, y_pos, (i % 2 == 0) ? win_msg[j] : ' ');

    // Play note
    play_note(luffy_theme[i].freq, luffy_theme[i].duration);
  }


  // ==== Phase 2: Now flash both messages ====
...
int main() {
    while (true) {
        ...
        // Clear PS/2 FIFO so music plays uninterrupted
        //while (!ps2.rx_fifo_empty()) (void)ps2.rx_byte();

        // Small delay for cleaner audio
        //sleep_ms(200);
        //play_mario_intro();
        //sleep_ms(300);  // rest before countdown
